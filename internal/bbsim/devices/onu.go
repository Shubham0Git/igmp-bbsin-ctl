...
func CreateONU(olt OltDevice, pon PonPort, id uint32, sTag int, cTag int, auth bool, dhcp bool) *Onu {
	...
	o.InternalState = fsm.NewFSM(
		...
		fsm.Events{
			...
			// IGMP
			{Name: "igmp_join_start", Src: []string{"enabled", "eap_response_success_received", "dhcp_ack_received"}, Dst: "igmp_join_started"},
			{Name: "igmp_join_done", Src: []string{"igmp_join_started"}, Dst: "igmp_join_done"},
			{Name: "igmp_join_error", Src: []string{"igmp_join_start"}, Dst: "igmp_join_error"},
			{Name: "igmp_leave", Src: []string{"igmp_join_done"}, Dst: "igmp_left"},
		},
		fsm.Callbacks{
